    ...
    public void validate(Object target, Errors errors) {
        ...
        if (price != null && price.length() > 0) {
            ...
            if (!price.matches("(^$|[0-9]*$)")){
                errors.rejectValue("price",  "price.number","Giá sản phẩm phải là số ,vui lòng nhập giá sản phẩm.");
                return;
            }
        ...
        if (quantity != null && quantity.length() > 0) {
            if (quantity.length() > 6){
                errors.rejectValue("quantity", "quantity.max","Số lượng sản phẩm tối đa là 999.999 ,vui lòng nhập giá sản phẩm." );
                return;
            }
